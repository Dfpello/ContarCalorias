...
def cargar_biblioteca():
    # Usamos string simple para que Streamlit pueda cachear la consulta sin errores
    return conn.query("SELECT * FROM biblioteca_alimentos ORDER BY comida ASC;", ttl="10m")

def cargar_log_hoy(fecha):
    # Pasamos la fecha como parámetro. Usamos string para evitar el error de hashing
        ...
        if seleccion:
            ...
            if st.button("Añadir al Diario"):
                ...
                with conn.session as s:
                    # Aquí SÍ usamos text() porque es una ejecución (s.execute)
                    s.execute(
